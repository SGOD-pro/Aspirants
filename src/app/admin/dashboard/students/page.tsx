...
const MemoizedTableRow = React.memo(({ student, onDelete }: { student: StudentWithId, onDelete: (id: string) => void }) => (
  <TableRow key={student.uid}>
    <TableCell className="font-medium uppercase">{student.studentId}</TableCell>
    ...
    <TableCell>
      <Button variant={"destructive"} size={"icon"} onClick={() => onDelete(student.uid!)}>
        <Trash2 />
      </Button>
    </TableCell>
  </TableRow>
...
function Students() {
  ...
  const renderedStudents = useMemo(() => (
    students?.map((student) => (
      <MemoizedTableRow key={student.uid} student={student} onDelete={deleteFunction} />
    ))
